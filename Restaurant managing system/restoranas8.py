...
WINDOW_SIZE_ORDERS = "600x400"

# GUI elementų dydžiai
LISTBOX_WIDTH = 45
LISTBOX_HEIGHT = 15
ORDERS_TEXT_WIDTH = 60
ORDERS_TEXT_HEIGHT = 20
...
# Pranešimai
MSG_ORDER_SUCCESS = "Užsakymas sėkmingai užbaigtas!"
MSG_ORDER_EMPTY = "Nėra ką užbaigti!"
MSG_LOGIN_SUCCESS = "Prisijungimas sėkmingas!"
MSG_LOGIN_ERROR = "Neteisingi prisijungimo duomenys!"
MSG_SHIFT_END = "Pamaina baigta. Viso gero!"
MSG_ADMIN_LOGOUT = "Administratorius atsijungė sėkmingai!"
MSG_PRODUCTS_ADDED = "Produktai pridėti į užsakymą!"
...
class RestaurantError(Exception):
    """Bazinė restorano sistemos išimtis."""
    pass

class OrderError(RestaurantError):
    """Išimtis užsakymų operacijoms."""
    pass

class EmployeeError(RestaurantError):
    """Išimtis darbuotojų operacijoms."""
    pass

class DataError(RestaurantError):
    """Išimtis duomenų operacijoms."""
    pass
...
class MenuItem:
    ...
    Attributes:
        name (str): Elemento pavadinimas
        price (float): Elemento kaina
    """
    
    def __init__(self, name: str, price: float) -> None:
        """Inicializuoja meniu elementą.
        
        Args:
            name: Elemento pavadinimas
            price: Elemento kaina
        """
        self.name = name
        self.price = price
    ...
    def get_display_info(self) -> str:
        ...
        Returns:
            str: Suformatuota elemento informacija
        """
...
class Pizza(MenuItem):
    ...
    Attributes:
        name (str): Picos pavadinimas
        price (float): Dabartinė picos kaina
        base_price (float): Bazinė picos kaina
        size (str): Picos dydis ('medium' arba 'large')
    """
    
    def __init__(self, name: str, base_price: float) -> None:
        """Inicializuoja picos objektą.
        
        Args:
            name: Picos pavadinimas
            base_price: Bazinė picos kaina
        """
    ...
    def set_size(self, size: str) -> None:
        ...
        Args:
            size: Picos dydis ('medium' arba 'large')
            
        Note:
            'large' dydis prideda 2€ prie bazinės kainos
        """
        self.size = size
        if size == "large":
    ...
    def get_display_info(self) -> str:
        ...
        Returns:
            str: Suformatuota picos informacija su dydžiu ir kaina
        """
...
def create_pizza_section(parent: tk.Frame) -> tuple[tk.Listbox, tk.StringVar]:
    """Sukuria picų sekciją.
    
    Args:
        parent: Tėvinis frame elementas
        
    Returns:
        tuple[tk.Listbox, tk.StringVar]: Picų sąrašo langas ir dydžio kintamasis
    """
    # Antraštė
    tk.Label(parent, text="Picos:", font=FONT_HEADER).pack()
    ...
    return pizza_listbox, size_var

def create_snack_section(parent: tk.Frame) -> tk.Listbox:
    """Sukuria užkandžių sekciją.
    
    Args:
        parent: Tėvinis frame elementas
        
    Returns:
        tk.Listbox: Užkandžių sąrašo langas
    """
    # Antraštė
    tk.Label(parent, text="Užkandžiai:", font=FONT_HEADER).pack()
    
    # Užkandžių frame su slinkjuostėmis
    snack_frame = tk.Frame(parent)
    snack_frame.pack(fill=tk.BOTH, expand=True)
    
    snack_scroll_x = tk.Scrollbar(snack_frame, orient=tk.HORIZONTAL)
    snack_scroll_y = tk.Scrollbar(snack_frame, orient=tk.VERTICAL)
    ...
    return snack_listbox

def create_drink_section(parent: tk.Frame) -> tk.Listbox:
    """Sukuria gėrimų sekciją.
    
    Args:
        parent: Tėvinis frame elementas
        
    Returns:
        tk.Listbox: Gėrimų sąrašo langas
    """
    # Antraštė
    tk.Label(parent, text="Gėrimai:", font=FONT_HEADER).pack()
...
def open_menu() -> None:
    ...
    try:
        ...
        menu_window.geometry(WINDOW_SIZE_MENU)
        ...
        # Sukuriame sekcijas
        pizza_listbox, size_var = create_pizza_section(frames['pizza'])
        snack_listbox = create_snack_section(frames['snack'])
        drink_listbox = create_drink_section(frames['drink'])
        ...
        def add_to_order() -> None:
            ...
            try:
                ...
                messagebox.showerror(
                    "Klaida",
                    "Nepavyko rasti pasirinkto produkto!"
                )
            except OrderError as e:
                messagebox.showerror(
                    "Klaida",
                    f"Klaida pridedant produktą: {str(e)}"
                )
...
def create_order_header(parent: tk.Widget) -> None:
    """Sukuria užsakymo lango antraštę.
    
    Args:
        parent: Tėvinis widget elementas
    """
    ...
    # Data ir laikas
    datetime_label = tk.Label(parent, font=FONT_NORMAL)
...
def create_order_list(parent: tk.Widget) -> tuple[tk.Listbox, tk.Label]:
    """Sukuria užsakymo sąrašo sekciją.
    
    Args:
        parent: Tėvinis widget elementas
        
    Returns:
        tuple[tk.Listbox, tk.Label]: Užsakymo sąrašo langas ir bendros sumos etiketė
    """
    # Užsakymo sąrašas
    ...
    return order_listbox, total_label

def create_order_buttons(parent: tk.Widget, total_label: tk.Label) -> None:
    """Sukuria užsakymo lango mygtukus.
    
    Args:
        parent: Tėvinis widget elementas
        total_label: Bendros sumos etiketė
    """
    # Mygtukų rėmelis
...
def open_order_window() -> None:
    ...
    try:
        if order_window is None or not order_window.winfo_exists():
            ...
            order_window.geometry(WINDOW_SIZE_ORDER)

            # Sukuriame lango komponentus
            create_order_header(order_window)
            order_listbox, total_label = create_order_list(order_window)
            create_order_buttons(order_window, total_label)
        ...
        messagebox.showerror(
            "Klaida",
            f"Nepavyko atidaryti užsakymo lango: {str(e)}"
        )
...
def end_shift(order_window: tk.Toplevel) -> None:
    ...
    messagebox.showinfo(MSG_SHIFT_END, MSG_SHIFT_END)
...
def finish_order() -> None:
    ...
    if system.finish_order(current_user):
        messagebox.showinfo(MSG_ORDER_SUCCESS, MSG_ORDER_SUCCESS)
        update_order_display()
        open_all_orders_window()
    else:
        messagebox.showwarning("Klaida", MSG_ORDER_EMPTY)
...
def open_all_orders_window() -> None:
    ...
    try:
        ...
        if all_orders_window is None or not all_orders_window.winfo_exists():
            ...
            orders_text = tk.Text(
                all_orders_window, 
                width=ORDERS_TEXT_WIDTH, 
                height=ORDERS_TEXT_HEIGHT, 
                wrap='word'
            )
        ...
        messagebox.showerror(
            "Klaida",
            f"Nepavyko atidaryti užsakymų lango: {str(e)}"
        )
...
def open_admin_window() -> None:
    ...
    try:
        ...
        admin_window.geometry(WINDOW_SIZE_ADMIN)
        ...
        def display_stats() -> None:
            ...
            try:
                ...
                messagebox.showerror("Klaida", f"Klaida nuskaitant duomenis: {str(e)}")
            except tk.TclError as e:
                messagebox.showerror("Klaida", f"GUI klaida: {str(e)}")
            except Exception as e:
                messagebox.showerror("Klaida", f"Netikėta klaida: {str(e)}")
        ...
        def admin_logout() -> None:
            ...
            try:
                ...
                username_entry.delete(0, tk.END)
                password_entry.delete(0, tk.END)
                messagebox.showinfo(MSG_ADMIN_LOGOUT, MSG_ADMIN_LOGOUT)
            except tk.TclError as e:
                messagebox.showerror("Klaida", f"GUI klaida: {str(e)}")
            except Exception as e:
                messagebox.showerror("Klaida", f"Netikėta klaida: {str(e)}")
...
def login() -> None:
    ...
    try:
        ...
        if not username or not password:
            raise ValueError("Neįvesti prisijungimo duomenys")
        
        if system.login_employee(username, password):
            current_user = username
            messagebox.showinfo(MSG_LOGIN_SUCCESS, MSG_LOGIN_SUCCESS)
            root.withdraw()
            ...
            raise EmployeeError("Neteisingi prisijungimo duomenys")
    except ValueError as e:
        messagebox.showerror("Klaida", str(e))
    except EmployeeError as e:
        messagebox.showerror(MSG_LOGIN_ERROR, str(e))
    except tk.TclError as e:
        messagebox.showerror("Klaida", f"GUI klaida: {str(e)}")
    except Exception as e:
        messagebox.showerror("Klaida", f"Netikėta klaida: {str(e)}")
...
def main() -> None:
    ...
    root = tk.Tk()
    root.geometry(WINDOW_SIZE_LOGIN)
